 ...
 *
 * This script creates a config file which works with the non-blocking calls:
 * DataRequest**** -> DataIsReady -> DataReceive****
 *
 * Compile this as:
                ...
                int nrhs, mxArray *prhs[])
{
    ...
    int number_of_markers = 3;
    float frame_rate = 100; //Hz.
    ...
    int are_we_streaming = 1; //stream the real-time data.
    ...
    float buffer_length = 1; //in seconds
    ...
    if(nrhs > 0)
    {
        mexErrMsgIdAndTxt("optotrakToolbox:dump_config_file_for_streaming_without_blocking:nrhs", "This function must not have an input argument.");
    }
    nlhs = 1;

    //For non-blocking, we need to set additional processing flags.
    fail = OptotrakSetProcessingFlags(OPTO_LIB_POLL_REAL_DATA | OPTO_CONVERT_ON_HOST | OPTO_RIGID_ON_HOST);
    if(fail)
    {
        mexErrMsgIdAndTxt("optotrakToolbox:dump_config_file_for_streaming_without_blocking:OptotrakSetProcessingFlags", "Couldn't set the processing flag.");
    }
    ...
    if(fail)
    {
        ...
        mexErrMsgIdAndTxt("optotrakToolbox:dump_config_file_for_streaming_without_blocking:OptotrakSetupCollection", "Couldn't set up the Optotrak system.");
    }

    //Assuming the software haven't failed, now we can dump the config file, which can be used.
    fail = OptotrakSaveCollectionToFile("stream_real_time_data_without_blocking");
    if(fail)
    {
        mexPrintf("ERROR:\n");
        //try tossing some error message out.
        ...
        mexErrMsgIdAndTxt("optotrakToolbox:dump_config_file_for_streaming_without_blocking:OptotrakSaveCollectionToFile", "Couldn't write config file..");
    }
