                ...
                int nrhs, mxArray *prhs[])
{
    ...
    if(fail)
    {
        ...
        mexErrMsgIdAndTxt("optotrakToolbox:dump_config_file_for_streaming_with_blocking:OptotrakSetupCollection", "Couldn't set up the Optotrak system.");
    }

    //Assuming the software haven't failed, now we can dump the config file, which can be used.
    fail = OptotrakSaveCollectionToFile("stream_real_time_data_with_blocking");
    if(fail)
    {
        mexPrintf("ERROR:\n");
        //try tossing some error message out.
        ...
        mexErrMsgIdAndTxt("optotrakToolbox:dump_config_file_for_streaming_with_blocking:OptotrakSaveCollectionToFile", "Couldn't write config file.");
    }
