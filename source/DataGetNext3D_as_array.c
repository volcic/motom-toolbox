/*
 * DataGetNext3D_as_array.c - Returns the captured Position3D data as a 2D array
 *
 *
 ...
 *
 * This is a wrapper for DataGetNext3D(), but it returns the Position3D data as a 2D array, as opposed to structure array.
 *
 * Compile this as:
                ...
                int nrhs, mxArray *prhs[])
{
    ...
    if(nrhs > 0)
    {
        mexErrMsgIdAndTxt("optotrakToolbox:DataGetNext3D_as_array:nrhs", "This function must must not have an input argument.");
    }
    nlhs = 4; //number of left-hand side arguments, which I set

    
    fail = DataGetNext3D(&frame_counter, &number_of_markers, &flags, &data_structure);
