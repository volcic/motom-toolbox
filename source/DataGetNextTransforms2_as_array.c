/*
 * DataGetNextTransforms2_as_array.c - Fetches the rigid body data just captured and converted.
 *
 *
 ...
 * note: The position 3D only returns the number of markers you specify as the input argument.
...
void mexFunction( int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    if(nrhs != 1)
    {
        mexErrMsgIdAndTxt("optotrakToolbox:DataGetNextTransforms2_as_array:nrhs", "This function must only have one input argument.");
    }
    //is the input argument a number?
    if( mxGetNumberOfElements(prhs[0]) != 1)
    {
        mexErrMsgIdAndTxt("optotrakToolbox:DataGetNextTransforms2_as_array:nrhs", "Input argument must be a scalar, Input argument must be a scalar, and has to be a round number.");
    }
    number_of_markers = mxGetScalar(prhs[0]);
    if(number_of_markers > 512 || number_of_markers < 0)
    {
        mexErrMsgIdAndTxt("optotrakToolbox:DataGetNextTransforms2_as_array:nrhs", "The system doesn't support more than 512 markers. You need to have at least one.");
    }
    nlhs = 6; //this is the number of output arguments


    fail = DataGetNextTransforms2(&frame_counter, &number_of_rigid_bodies, &flags, &rigid_transforms, &positions);
    if(number_of_rigid_bodies == 0)
    {
        mexErrMsgIdAndTxt("optotrakToolbox:DataGetNextTransforms2_as_array:nrhs", "There were no rigid bodies defined in the system!");
    }
    ...
    */
    ...
    for(i=0; i<number_of_rigid_bodies; i++)
    {
        ...
        {
            mexErrMsgIdAndTxt("optotrakToolbox:DataGetNextTransforms2_as_array:frame_converter_loop", "The rigid body transformation method must be euler. Use RigidBodyAddFromFile_euler(), for example.");
        }
