/*
 * Read the Optotrak raw data file, and then get the position3d information out of it.
 *
                ...
                int nrhs, mxArray *prhs[])
{
    ...
    if(nrhs != 1)
    {
        mexErrMsgIdAndTxt("optotakToolbox:convert_raw_file_to_position3d_array:nrhs", "This function must have only one input argument.");
    }
    //both input arguments must be a string.
    if( !mxIsChar(prhs[0]))
    {
        mexErrMsgIdAndTxt("optotakToolbox:convert_raw_file_to_position3d_array:nrhs", "Make sure that the input argument is a string!");
    }
    //...and both strings must have something in it.
    if( mxGetNumberOfElements(prhs[0]) < 2)
    {
        mexErrMsgIdAndTxt("optotakToolbox:convert_raw_file_to_position3d_array:nrhs", "The input file name can't be empty!");
    }
    ...
    if(OptotrakGetStatus(NULL, NULL, NULL, &number_of_markers, &frame_rate, NULL, NULL,  NULL, NULL, NULL, NULL, &collection_time, NULL, NULL))
    {
        ...
        {
            mexPrintf("%s\n", error_string);
            mexErrMsgIdAndTxt("optotakToolbox:convert_raw_file_to_position3d_array:OptotrakGetStatus", "Couldn't read system status. The system must be initialised properly before it can convert data.");
        }
        else
        {
            mexErrMsgIdAndTxt("optotakToolbox:convert_raw_file_to_position3d_array:OptotrakGetErrorString", "Something went so wrong, even the error message printer function failed.");
        }
    }
    ...
    if(FileOpen(mxArrayToString(prhs[0]), file_id, file_mode, &number_of_items, &number_of_subitems, &number_of_frames, &frame_frequency, &comments, &file_pointer))
    {
        ...
        {
            mexPrintf("%s\n", error_string);
            mexErrMsgIdAndTxt("optotakToolbox:convert_raw_file_to_position3d_array:FileOpen", "Couldn't load raw data file. Does it exist? Have you got permission? (did you make a typo in the file name?)");
        }
        else
        {
            mexErrMsgIdAndTxt("optotakToolbox:convert_raw_file_to_position3d_array:OptotrakGetErrorString", "Something went so wrong, even the error message printer function failed.");
    ...
    for(i=0; i<number_of_frames; i++)
    {
        ...
        if(FileRead(file_id, i, 1, one_frame_data)) //read 1 data frame to the allocated memory.
        {
            ...
            {
                mexPrintf("%s\n", error_string);
                mexErrMsgIdAndTxt("optotakToolbox:convert_raw_file_to_position3d_array:FileRead", "The file was opened, but couldn't be read.");
            }
            else
            {
                mexErrMsgIdAndTxt("optotakToolbox:convert_raw_file_to_position3d_array:OptotrakGetErrorString", "Something went so wrong, even the error message printer function failed.");
        ...
        if(OptotrakConvertRawTo3D(&number_of_elements, one_frame_data, &converted_positions)) //need to watch for pointers here.
        {
            ...
            {
                mexPrintf("%s\n", error_string);
                mexErrMsgIdAndTxt("optotakToolbox:convert_raw_file_to_position3d_array:OptotrakConvertRawTo3D", "Couldn't convert raw data to positions.");
            }
            else
            {
                mexErrMsgIdAndTxt("optotakToolbox:convert_raw_file_to_position3d_array:OptotrakGetErrorString", "Something went so wrong, even the error message printer function failed.");
        ...
        {
            mexErrMsgIdAndTxt("optotakToolbox:convert_raw_file_to_position3d_array:frame_converter_loop", "The number of elements in a raw frame do not match with the number of markers initialised in the system.");
        }
