...
%   So, 0 for all good, and pretty much anything else for fail.
    ...
    if(isunix)
        fail = calllib('liboapi', 'DataGetNextCentroid', uFrameNumber_pointer, uElements_pointer, uFlags_pointer, DataDest_pointer);
    else
        if(new_or_old)
            fail = calllib('oapi64', 'DataGetNextCentroid', uFrameNumber_pointer, uElements_pointer, uFlags_pointer, DataDest_pointer);
        else
            fail = calllib('oapi', 'DataGetNextCentroid', uFrameNumber_pointer, uElements_pointer, uFlags_pointer, DataDest_pointer);
        end
    end
