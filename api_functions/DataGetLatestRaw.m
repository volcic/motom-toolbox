...
%DATAGETLATESTRAW
% [ fail, puFrameNumber, puElements, puFlags, pDataDest ] = DataGetLatestRaw( puFrameNumber, puElements, puFlags, pDataDest )
% This function gets you the latest acquired raw data
...
%   So, 0 for all good, and pretty much anything else for fail.
    ...
    if(isunix)
        fail = calllib('liboapi', 'DataGetLatestRaw', uFrameNumber_pointer, uElements_pointer, uFlags_pointer, DataDest_pointer);
    else
        if(new_or_old)
            fail = calllib('oapi64', 'DataGetLatestRaw', uFrameNumber_pointer, uElements_pointer, uFlags_pointer, DataDest_pointer);
        else
            fail = calllib('oapi', 'DataGetLatestRaw', uFrameNumber_pointer, uElements_pointer, uFlags_pointer, DataDest_pointer);
        end
    end
